...
[Sangiorgi2011]. -/
@[lts CCS.lts]
inductive Tr [DecidableEq Name] :
    Process Name Constant → Act Name → Process Name Constant → Prop where
  | pre : Tr (pre μ p) μ p
  | parL : Tr p μ p' → Tr (par p q) μ (par p' q)
  | parR : Tr q μ q' → Tr (par p q) μ (par p q')
  | com :
    Tr p μ p' → Tr q μ' q' → Act.IsDual (Name := Name) μ μ' → Tr (par p q) Act.τ (par p' q')
